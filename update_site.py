...
# T-138 (02.09.2026, по скриншоту пользователя — тумблеры в калькуляторе
# визуально "не уходят" в положение "включено", хотя в HTML стоит
# checked). Причина не в разметке и не в JS (проверено напрямую в
# браузере: computed style у чекбокса с checked=true даёт правильный
# зелёный фон и сдвинутый бегунок) — HTML/JS/CSS в репозитории на момент
# проверки согласованы между собой. Настоящая причина: <link
# href="style.css"> и <script src="app.js"> отдавались БЕЗ версии в URL —
# браузер (и любой промежуточный кэш) мог держать старую версию файла
# сколько угодно после деплоя, до жёсткого обновления страницы
# пользователем. За один день style.css/app.js правились ~15 раз подряд
# (T-126..T-137) — при такой частоте деплоев рассинхрон "свежий HTML +
# устаревший закэшированный CSS/JS" почти неизбежен на любом клиенте,
# который не обновляет страницу вручную. Версия в URL — короткий md5-хэш
# от актуального содержимого файла: меняется сам собой при каждой правке
# style.css/app.js, вручную бампать не нужно, и не ломается, если файл не
# менялся (тот же хэш — тот же URL — эффективно использует обычный
# HTTP-кэш браузера, просто безопасно инвалидирует его при реальном
# изменении). Регулярка сама подхватывает уже существующий "?v=..." (если
# он появится от прошлого прогона) и заменяет его целиком, а не плодит
# "?v=...?v=...".
def _asset_version(path):
    with open(path, "rb") as f:
        return hashlib.md5(f.read()).hexdigest()[:10]

_style_v = _asset_version("style.css")
_app_js_v = _asset_version("app.js")
html = re.sub(r'href="style\.css(?:\?v=[^"]*)?"', f'href="style.css?v={_style_v}"', html)
html = re.sub(r'src="app\.js(?:\?v=[^"]*)?"', f'src="app.js?v={_app_js_v}"', html)
