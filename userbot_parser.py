...
"фиаско полное, посмотри какие огромные закраски бейджа"): remove_watermark()
для winner_auto_club ОТКЛЮЧЕНА (PHOTO_PROCESSORS пуст) — на новых реальных
фото template matching ни разу не нашёл настоящий бейдж, вместо этого с
уверенностью 0.55-0.62 закрашивал случайные места (фон-штору, и даже фото
салона без бейджа вообще). winner_auto_club временно добавлен в
TEST_ONLY_SOURCES (фото идут в тест как есть, с видимым чужим бейджем) —
до тех пор, пока распознавание не переделают и не проверят на реальном
трафике. Функция и шаблоны оставлены в коде для доработки.
...
- winner_auto_club (добавлен 25.08.2026): задумано автоматическое удаление
  водяного знака канала с фото (согласовано с собственником, template
  matching + заливка, см. remove_watermark ниже) — но по факту (T-85,
  26.08.2026) на реальных фото не находит бейдж вообще, отключено
  (PHOTO_PROCESSORS пуст), источник временно в TEST_ONLY_SOURCES. Видео с
  тем же знаком тоже не обрабатывается — публикуется как есть, и поэтому
  такие посты принудительно идут в тестовую группу через VIDEO_DRY_RUN
  (см. выше) — сейчас, впрочем, это уже неактуально, весь источник и так в
  тесте через TEST_ONLY_SOURCES.
"""
import asyncio
import io
import json
...
# заранее масштабе/позиции, затем область закрашивается сплошным цветом
# (средний цвет самой области).
#
# Перебрали два других способа перед этим, оба не подошли:
#   1. cv2.inpaint (INPAINT_TELEA) — неплохо работал там, где вокруг бейджа
#      была текстура для "дорисовки" (решётка радиатора), но бейдж почти
#      всегда висит именно на месте номера — там заведомо нет текстуры для
#      реконструкции, и inpaint давал плоское мутное пятно ("замылил",
#      жалоба пользователя 25.08.2026).
#   2. Сильный Gaussian blur — лучше, но даже большое ядро блюра не убирает
#      крупные цветовые пятна: белый/зелёный/белый текст на тёмном фоне
#      остаётся виден как размытый, но узнаваемый силуэт логотипа ("за
#      блюром проглядывается логотип винера", та же жалоба).
# Сплошная заливка полностью убирает любой узор — заливке нечем "просвечивать".
#
...
_WATERMARK_PAD = 8  # px, минимальный отступ вокруг найденного бокса перед блюром
...
def remove_watermark(image_bytes):
    ...
    выходе) сплошной заливкой найденной области. Если знак не найден уверенно
    ...
    x, y, w, h, score = box
    # Реальный размер физической рамки на конкретном фото не всегда точно
    # совпадает с масштабом, на котором сработал template match — 25.08.2026
    # поймали случай, где фиксированного pad=8 не хватило и сверху бейджа
    # осталась читаемая полоска текста. Берём pad от размера самого бокса
    # (с полом от _WATERMARK_PAD). Первая версия (w//5, h//3) перекрывала
    # заметно больше, чем нужно (та же жалоба 25.08.2026, "закрашивает
    # больше чем нужно") — уменьшили вдвое, этого достаточно, чтобы закрыть
    # редкий недохват по краю, не закрашивая лишний кузов вокруг.
    pad_x = max(_WATERMARK_PAD, w // 10)
    pad_y = max(_WATERMARK_PAD, h // 6)
    x0, y0 = max(0, x - pad_x), max(0, y - pad_y)
    x1, y1 = min(img.shape[1], x + w + pad_x), min(img.shape[0], y + h + pad_y)

    roi = img[y0:y1, x0:x1]
    # Даже очень сильный Gaussian blur не убирает бейдж полностью — крупные
    # цветовые пятна (белый/зелёный/белый текст на тёмном фоне) остаются
    # видны как размытый, но узнаваемый силуэт логотипа (жалоба пользователя
    # 25.08.2026: "за блюром проглядывается логотип винера"). Поэтому вместо
    # блюра заливаем всю область одним сплошным цветом — средним цветом
    # самой области. Это гарантированно убирает любой узор: заливка не
    # может "просвечивать", в отличие от блюра.
...
#
# T-85 (26.08.2026, пользователь прислал реальные фото из боевого запуска —
# "фиаско полное, посмотри какие огромные закраски бейджа"): remove_watermark()
# ОТКЛЮЧЕНА для winner_auto_club — на новых фото template matching (см. выше)
# ни разу не нашёл настоящий бейдж (лучшее совпадение 0.55-0.62 стабильно
# попадает на однотонный фон-шторку или даже на фото салона, где бейджа нет
# вообще — проверено прогоном реальных фото через _find_best_watermark_box()).
# То есть сейчас функция не прячет бейдж, а портит случайные места на КАЖДОМ
# фото. Пока в проде фото идут БЕЗ обработки — как есть, с видимым чужим
# бейджем (winner_auto_club также добавлен в TEST_ONLY_SOURCES, см. main(),
# пока результат не пересмотрен). Саму функцию remove_watermark() и шаблоны
# не удаляем — понадобятся, когда подход будет переделан (см. TASKS.md T-85).
PHOTO_PROCESSORS = {}
...
async def main():
    ...
    test_only_sources = {s.strip().lstrip("@").lower() for s in env.get("TEST_ONLY_SOURCES", "tamsyam26,winner_auto_club").split(",") if s.strip()}
