...
async def handle_group(client, source_username, messages, targets_cfg, eur_rub_rate, usd_rub_rate, dry_run, video_dry_run, test_group, state, feedback_bot_username=None, test_only_sources=frozenset(), video_dry_run_sources=frozenset({"winner_auto_club"}), preposting_group=None):
    ...
    if montage_used:
        # T-172 (13.09.2026, запрошено пользователем — "видео из источника
        # идет в автомонтаж, далее в боевые группы, автовинер пока в
        # тестовую, от тамсяма видео не берем"): отменяет T-160. Раньше
        # смонтированный ролик ВСЕГДА уходил ТОЛЬКО в группу предпостинга
        # (даже для artalexgroup/bezpokrasa, которым текст/фото и так уже
        # шли в боевые) — пользователь подтвердил, что ручной отбор больше
        # не нужен: ролик просто заменяет исходное видео в том же посте и
        # идёт туда же, куда шёл бы этот пост без монтажа. Поэтому здесь
        # больше НЕ меняем ни post_text (остаётся обычный build_repost_text,
        # посчитанный выше — со ссылкой на бота, которую tg-sync унесёт
        # дальше в VK/MAX/Instagram-ленту как обычный репост), ни
        # send_targets: artalexgroup/bezpokrasa получат обычные боевые цели
        # (ветка выше это уже посчитала), winner_auto_club — тестовую (он
        # по-прежнему целиком в TEST_ONLY_SOURCES, эта ветка его не трогает).
        # PREPOSTING_GROUP_INVITE для видео больше не используется —
        # параметр preposting_group остаётся нужен только для карточки
        # «Предложение дня» (T-169/T-170), не для этой ветки.
        logger.info(
            "[%s#%s] видео прошло через auto_montage, публикуется обычным путём -> %s",
            source_username, ids, send_targets,
        )
