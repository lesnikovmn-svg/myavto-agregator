...
TRACK_SCORE_THRESH = 0.65  # self-template match — планка выше, чем для generic
                           # template matching (0.55 у фото-детектора v1), т.к.
                           # здесь матчим кадр против себя же несколько кадров
                           # назад, совпадение должно быть намного увереннее
...
FULL_PLATE_AR = 5.5        # запас по ширине области закрытия — OCR обычно ловит
...
def _badge_video_regions(img):
    ...
    h, w = img.shape[:2]
    return ((0, int(h * 0.55), w, int(h * 0.92)),)
...
def ocr_anchor(frame_bgr):
    ...
    return up._find_badge_box(frame_bgr, regions=_badge_video_regions(frame_bgr))
